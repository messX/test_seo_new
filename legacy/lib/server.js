...
var PAGE_DONE_CHECK_TIMEOUT = process.env.PAGE_DONE_CHECK_TIMEOUT || 15000;

var RESOURCE_DOWNLOAD_TIMEOUT = process.env.RESOURCE_DOWNLOAD_TIMEOUT || 10 * 1000;

var WAIT_AFTER_LAST_REQUEST = process.env.WAIT_AFTER_LAST_REQUEST || 5000;

var JS_CHECK_TIMEOUT = process.env.JS_CHECK_TIMEOUT || 5000;

var JS_TIMEOUT = process.env.JS_TIMEOUT || 10 * 1500;

var NO_JS_EXECUTION_TIMEOUT = process.env.NO_JS_EXECUTION_TIMEOUT || 5000;

var EVALUATE_JAVASCRIPT_CHECK_TIMEOUT = process.env.EVALUATE_JAVASCRIPT_CHECK_TIMEOUT || 5000;
...
server._pluginEvent = function(methodName, pageType, args, callback) {
    var _this = this
      , index = 0
      , next;
    // console.log('started - ', methodName, '\n');
    ...
    next = function() {
        ...
        if(layer.allowedPageTypes == undefined){
            layer.allowedPageTypes = [];
        }

        var allPagesAllowed = false;

        if(layer.allowedPageTypes.length == 1 && layer.allowedPageTypes[0] == '*'){
            allPagesAllowed = true;
        }

        if (method && (allPagesAllowed || _.findIndex(layer.allowedPageTypes, function(pt){return pt == pageType}) != -1)){
            // console.log('apply - ', layer.name);
            method.apply(layer, args);
        } else {
            // console.log('skip - ', layer.name);
            next();
        }
    };
...
server.onRequest = function(req, res) {
    ...
    var metaTags = util.generateMetaData(req);

    req.prerender = {
        url: metaTags.url,
        start: new Date(),
        pageDoneCheckTimeout: metaTags.pageDoneCheckTimeout,
        pageType: metaTags.pageType,
        enableSnapshot: metaTags.enableSnapshot
    };
    console.log('arguments - ', req.prerender);

    // console.log('getting', req.prerender.url);

    this._pluginEvent("beforePhantomRequest",  req.prerender.pageType, [req, res], function() {
...
server.onPhantomPageCreate = function(req, res) {
    ...
    _this._pluginEvent("onPhantomPageCreate",  req.prerender.pageType, [_this.phantom, req, res], function() {
...
server.onPageEvaluate = function(req, res) {
    ...
    if (!req.prerender.documentHTML) {
        ...
        this._pluginEvent("afterPhantomRequest",  req.prerender.pageType, [req, res], function() {
...
server._send = function(req, res, statusCode, options) {
    ...
    this._pluginEvent("beforeSend",  req.prerender.pageType, [req, res], function() {
...
server._disposeAll = function(callback) {
    ...
    var _this = this
      , disposed = false
      , phantomPid = this.phantom.childProcess.pid;
