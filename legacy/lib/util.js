...
util.getUrl = function (req) {
    ...
    realUrl = realUrl.substring(realUrl.indexOf('http:'));
    util.log('---------------' + realUrl);
...
util.generateMetaData = function(req){
    var url = util.getUrl(req);
    var pageDoneCheckTimeout = 7000;
    var pageType = '';
    var takeSnapshot = false;
    if(_.includes(req.url, 'snapshot')){
        takeSnapshot = true;
        console.log('snapshot enabled....')
    }
    if(_.includes(req.url, 'list'))
    {
        pageDoneCheckTimeout = 15000;
        pageType = 'list'
    }
    else if(_.includes(req.url, 'detail')){
        // pageDoneCheckTimeout = 7000;
        pageType = 'detail'
    }

    return {
        url: url,
        pageDoneCheckTimeout: pageDoneCheckTimeout,
        pageType: pageType,
        enableSnapshot: takeSnapshot
    }
};